...
# zmodload zsh/zprof
# Dummy testing.
# alias gpmt="rmnm && rmv package.json rmv package-lock.json && npm init -y && rmv yarn.lock && rmv yarn-error.log && /Users/$USER/.aCHANGEsdf/installs/nodejs/12.13.0/.npm/bin/gpm"
...
# Homebrew env — inlined from `brew shellenv` to avoid spawning brew every startup.
# Regenerate if Homebrew ever moves: run `brew shellenv` and paste the output here.
...
# -- Sync: back up ALL dotfiles to PRIVATE repo. Public repo is committed manually (verified-safe snapshots). --
alias syncd="syncdfiles"
syncdfiles() {
	echo "\n${yf}❯ SYNCING DOTFILES${r}"
	local priv="$HOME/dotfiles-private"
	local KEYRE='AKIA[0-9A-Z]{16}|ghp_[A-Za-z0-9]{36}|gho_[A-Za-z0-9]{30}|sk-[A-Za-z0-9]{20}|xox[baprs]-[A-Za-z0-9-]{10}|AIza[0-9A-Za-z_-]{35}|BEGIN [A-Z ]*PRIVATE KEY'

	# ---- PRIVATE: shell/git config + machine backups ----
	if [ ! -d "$priv/.git" ]; then git clone https://github.com/ahmadawais/dotfiles-private.git "$priv" || { echo "${rf}❯ private clone failed${r}"; return 1; }; fi
	( cd "$priv" || return 1
	  git pull --ff-only >/dev/null 2>&1
	  rsync -a ~/.zshrc ~/.bashrc ~/.bash_profile ~/.gitconfig "$priv/" 2>/dev/null
	  rsync -a "$HOME/Library/Application Support/Code/User/spellright.dict" "$priv/vscode/" 2>/dev/null
	  brew bundle dump --file="$priv/Brewfile" --force >/dev/null 2>&1
	  npm ls -g --depth=0 2>/dev/null | sed '1d' | awk '{print $2}' | sed '/^$/d' > "$priv/npm-global.txt"
	  rmds >/dev/null 2>&1
	  git add -A
	  if git diff --cached | grep -iEq "$KEYRE"; then
	    echo "${rb}${wf} STOP: secret-shaped string in PRIVATE diff - aborting. cd $priv && git diff --cached ${r}"; return 1
	  fi
	  if git diff --cached --quiet; then echo "${gf}❯ private: no change${r}"
	  else git commit -q -m "sync: dotfiles-private ($(hostname -s))" && git push -q origin main && echo "${gf}❯ private -> dotfiles-private${r}" || echo "${rf}❯ private push failed${r}"
	  fi )
	echo "${gf}✅ DONE${r}\n"
}
alias dotfiles="cd ~/dotfiles"

# -- syncpub: deliberately PUBLISH verified-safe dotfiles to the public repo (preview + secret gate + confirm) --
alias syncpub="syncpublic"
syncpublic() {
	echo "\n${yf}❯ PUBLISH → public dotfiles${r}"
	local pub="$HOME/dotfiles"
	local KEYRE='AKIA[0-9A-Z]{16}|ghp_[A-Za-z0-9]{36}|gho_[A-Za-z0-9]{30}|sk-[A-Za-z0-9]{20}|xox[baprs]-[A-Za-z0-9-]{10}|AIza[0-9A-Za-z_-]{35}|BEGIN [A-Z ]*PRIVATE KEY'
	[ -d "$pub/.git" ] || { echo "${rf}❯ $pub is not a git repo${r}"; return 1; }
	( cd "$pub" || return 1
	  git pull --ff-only >/dev/null 2>&1
	  rsync -a ~/.zshrc "$pub/.zshrc" 2>/dev/null
	  rsync -a ~/.gitconfig "$pub/.gitconfig" 2>/dev/null
	  rmds >/dev/null 2>&1
	  git add -A
	  if git diff --cached --quiet; then echo "${gf}❯ Already up to date — nothing to publish.${r}"; return 0; fi
	  echo "${yf}❯ Will publish these changes:${r}"
	  git diff --cached --stat
	  if git diff --cached | grep -iEq "$KEYRE"; then
	    echo "${rb}${wf} STOP: secret-shaped string detected — NOT pushing. Review: cd $pub && git diff --cached ${r}"
	    return 1
	  fi
	  if read -q "ans?${yf}❯ Push to PUBLIC repo? [y/N] ${r}"; then
	    echo
	    git commit -q -m "update: dotfiles ($(date +%F))" && git push -q origin main && echo "${gf}❯ Published → github.com/ahmadawais/dotfiles${r}" || echo "${rf}❯ push failed${r}"
	  else
	    echo "\n${yf}❯ Cancelled — nothing pushed (staged changes left for review).${r}"
	  fi )
	echo "${gf}✅ DONE${r}\n"
}
...
#  cdf: cd to the frontmost Finder window's folder (function above; safe if no
#  window is open — falls back to Desktop).
#  cdfa: the old `cdf` alias, renamed. Uses `target of window 1`; errors if no
#  Finder window is open. Kept because it differs from cdf in that edge case.
alias cdfa='cd "$(osascript -e '\''tell app "Finder" to return the POSIX path of (target of window 1 as alias)'\'')"'
...
# Empty mail.
alias emptymail="echo > /var/mail/$USER && echo 'DONE!!! 💯'"
...
bkpics() {
	...
	echo "—————————————————————————————"
	RSP_PARAM="-av"
	BKP_DST="/Volumes/AhmadAwais.com/BACKUPS/Users/$USER/Pictures"
...
# Run QB64.
alias qb="/Users/$USER/Documents/QB/qb64/qb64_start_osx.command"
...
# File size in raw bytes. Renamed from `fs` to `fsb` because `fs` is also a
# function above (human-readable du size); the name clash caused a startup parse
# error. Use `fs` for human-readable size, `fsb` for exact bytes.
alias fsb="stat -f \"%z bytes\""
...
alias cppp="cat /Users/$USER/cpapi/class/PPP.php"

ppp() {
	echo '\n'
	grep "'$1'" /Users/$USER/cpapi/class/PPP.php
	echo '\n'
}
...
# Delete this.
alias create-cloudinary-headshot="/Users/$USER/work/headshot/cli/index.js"
...
# Generate Change log.
clog() {
	auto-changelog --package --unreleased --output 'changelog.md' --commit-limit 'false' --template "/Users/$USER/Dropbox/bin/changelog.hbs" --config "/Users/$USER/Dropbox/bin/.auto-changelog"
	git add . && git commit -m ' 📖 DOC: Changelog' && git push
}

# Generate Change log without ads.
clogx() {
	auto-changelog --package --unreleased --output 'changelog.md' --commit-limit 'false' --template "/Users/$USER/Dropbox/bin/changelogx.hbs" --config "/Users/$USER/Dropbox/bin/.auto-changelog"
	git add . && git commit -m ' 📖 DOC: Changelog' && git push
}
...
lbo() {
	code "/Users/$USER/Library/CloudStorage/Dropbox/_Langbase/Dev/langbase.code-workspace"
}
...
gprmr() {
	# Draft PRs can't be merged — flip to ready first if needed.
	local is_draft
	is_draft=$(gh pr view --json isDraft --jq '.isDraft' 2>/dev/null)
	if [[ "$is_draft" == "true" ]]; then
		echo "${bf}❯ Draft PR — marking ready for review…${r}"
		gh pr ready
	fi
...
obsbackup() {
  local dir="/Users/$USER/Documents/Sandbox/del/aa-obs"
...
# <<< microsandbox <<<
# CF CLI completions
[[ -f "/Users/$USER/.config/cf/completions/_cf.zsh" ]] && source "/Users/$USER/.config/cf/completions/_cf.zsh"
...
# transcribe <video-url> — extract the audio into its own folder in ~/Downloads,
# transcribe it with parakeet-mlx (all formats), then open the folder.
transcribe () {
	emulate -L zsh
	local url="$1"
	if [[ -z "$url" ]]; then
		print -u2 "Usage: transcribe <video-url>"
		return 1
	fi

	# Resolve the sanitized video title yt-dlp will use as the filename.
	print "▸ Fetching video info…"
	local title
	title=$(yt-dlp --get-filename -o '%(title)s' "$url") || {
		print -u2 "transcribe: could not fetch video title"
		return 1
	}

	local dir="$HOME/Downloads/$title"
	mkdir -p "$dir" || return 1

	# Download audio only (no video), extracted to mp3, into the folder.
	# Echo the real command, then stream its live output below.
	local -a dl_cmd=(yt-dlp -x --audio-format mp3 --newline -o "$dir/%(title)s.%(ext)s" "$url")
	print -Pn "%F{cyan}⎿ DOWNLOADING%f "; print -r -- "${(q-)dl_cmd[@]}"
	"${dl_cmd[@]}" || {
		print -u2 "transcribe: audio download failed"
		return 1
	}

	# Prefer the expected file; fall back to the newest audio file in the folder.
	local audio="$dir/$title.mp3"
	if [[ ! -f "$audio" ]]; then
		audio=$(print -r -- "$dir"/*.(mp3|m4a|wav|flac|opus|aac|ogg)(om[1]N))
	fi
	if [[ -z "$audio" || ! -f "$audio" ]]; then
		print -u2 "transcribe: no audio file found in $dir"
		return 1
	fi

	# Transcribe; echo the real command, then stream parakeet-mlx output live.
	local -a tr_cmd=(parakeet-mlx --output-format all --output-dir "$dir" "$audio")
	print -Pn "%F{cyan}⎿ TRANSCRIBING%f "; print -r -- "${(q-)tr_cmd[@]}"
	"${tr_cmd[@]}" || {
		print -u2 "transcribe: transcription failed"
		return 1
	}

	print "▸ Done → opening $dir"
	open "$dir"
}
