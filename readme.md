...
2. **Configure zsh** - Add plugins to `~/.zshrc`:
   ...
   ```
3. **Check [`setup/manual.md`](setup/manual.md)** for:
...
├── .zshrc                # My zsh config (secrets load from outside the repo)
├── .gitconfig            # Git config, aliases, signing (no tokens — keychain handles auth)
├── setup/
│   ├── manual.md         # Post-install manual steps
│   └── old-apps.md       # Optional / removed apps
├── raycast/              # Raycast window-layout scripts
├── editor/               # .editorconfig + .prettierrc
├── macos/preferences/    # Spectacle window-management prefs
└── readme.md             # This file
```

> `.zshrc` is safe to be public — all secrets are sourced at runtime from files kept **outside** this repo. Other machine config/backups live in a separate private repo. No secrets here.
